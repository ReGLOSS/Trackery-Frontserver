...
export class MapManager {
    ...
    // 캐시된 데이터로 시도 스타일 적용
    applyCachedSidoStyles(cachedData) {
    ...
    // 지도 스타일 초기화
    resetMapStyles() {
        const pathElements = document.querySelectorAll('#map-display path');
        pathElements.forEach(element => {
            element.classList.remove('coverage-complete', 'coverage-partial');
        });
    }

    // 에러 전파 여부 판단
    shouldPropagateError(error) {
        // 네트워크 에러는 전파하지 않음 (폴백 처리됨), 다른 에러는 전파
        return !(error.message.includes('Failed to fetch') || error.name === 'NetworkError');
    }

    // 시도 스타일링 에러 처리
    handleSidoStyleError(error) {
        ...
        if (error.message.includes('Failed to fetch')) {
            ...
            if (cachedData?.sido) {
                this.applyCachedSidoStyles(cachedData.sido);
                this.showMapUpdateNotification('캐시된 데이터로 지도를 표시합니다.', 'warning');
                return;
            }
        }
    ...
    async styleSidosWithImages() {
        ...
        try {
            // 공통 메서드 사용
            const coverageData = await this.fetchSidoCoverageData();
            
            // 캐시 업데이트
            this.updateImageCacheWithSidoData(coverageData);
            
            // 스타일 적용
            this.applySidoStyles(coverageData);
            
        } catch (error) {
            // 개선된 에러 처리
            this.handleSidoStyleError(error);
        }
    }
    ...
    async updateSidoImageCache() {
        ...
        try {
            ...
            // 캐시 업데이트 실패는 치명적이지 않으므로 에러를 전파하지 않음
        }
    }
